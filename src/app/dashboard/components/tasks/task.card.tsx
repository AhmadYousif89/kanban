...
const TaskCardBody = ({ task }: { task: Task }) => {
  ...
  return (
    <>
      ...
      <span className='mt-2 text-xs font-bold text-muted-foreground block'>
...
export const TaskCardPreview = ({ task, className }: { task: Task; className?: string }) => {
  return (
    <div
      className={cn(
        'flex flex-col w-full wrap-break-word rounded-lg bg-card px-4 py-6 text-left drop-shadow-lg drop-shadow-primary/25',
        className,
      )}
    >
      <TaskCardBody task={task} />
